...
@push('head')
    <style>
        ...
        .button-category:hover{
            ...
            background-color: {{ env('PRIMARY_COLOR').'33' }};
        }
        ...
        <div class="container px-4 mx-auto">
            <div class="md:max-w-5xl mx-auto mb-8 md:mb-16 text-center">
                <h3 class="mb-4 block-title font-black font-heading leading-tight text-gray-900 tracking-tighter">
                    @if(env('APP_NAME') == 'AABB-SP')
                        Aconteceu na <span style="color: {{ env('PRIMARY_COLOR') }}">AABB</span>.
                    @else
                        Acompanhe nosso <span style="color: {{ env('PRIMARY_COLOR') }}">BLOG</span>.
                        @if(env('APP_NAME') == 'Ultra Lims')
                            <br>Sempre novidades sobre o mundo Lims.
                        @endif
                    @endif
                </h3>
            ...
            <ul class="flex flex-wrap mb-8 -mx-2 text-center">
                <li class="w-full md:w-auto px-2">
                    <a href="{{ route('blog') }}" class="inline-block w-full py-2 px-4 mb-4 md:mb-0 text-sm {{ !isset($_GET['categoria']) ? 'button-category-active' : 'text-gray-400' }} button-category font-bold rounded-xl hover:shadow-sm">Todos ({{ $articlesCount }})</a>
                </li>
                @foreach ($categorias as $categoria)
                    <li class="w-full md:w-auto px-2">
                        <a href="/blog?categoria={{ $categoria->slug }}" class="inline-block w-full py-2 px-4 mb-4 md:mb-0 text-sm {{ isset($_GET['categoria']) ? ($_GET['categoria'] == $categoria->slug ? 'button-category-active' : 'text-gray-400') : 'text-gray-400' }} button-category font-bold rounded-xl hover:shadow-sm">{{ $categoria->title }} ({{ $categoria->articles_count }})</a>
                    </li>
                @endforeach
            </ul>
            <div class="articles-container flex flex-wrap -mx-4 mb-12 md:mb-20">
                @include('site.pages.blog.inc.articles', ['articles' => $articles])
            </div>
            @if($articles->count() == 6)
