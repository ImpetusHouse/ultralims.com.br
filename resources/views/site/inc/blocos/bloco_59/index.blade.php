...
@endpush
<section id="section_{{ $i }}" class="overflow-hidden" style="background-color: {{ $block->background_color }}; padding-top: {{ $block->margin_top }}; padding-bottom: {{ $block->margin_bottom }}">
    <!-- Swiper -->
                ...
                <div class="swiper-slide">
                    <div class="flex justify-center w-full h-full">
                        <div class="relative h-[500px] w-full overflow-hidden">
                            <img class="h-[500px] w-full object-cover transform hover:scale-105 transition ease-in-out duration-500" src="{{ asset(str_replace('public/', 'storage/', $topic->path)) }}" alt="{{ $topic->title }}"/>
                            <div class="absolute bottom-0 left-0 w-full">
                                <div class="px-8 py-4">
                                    <h3 class="text-lg text-left font-semibold" style="color: {{ $block->title_color }};">{{ $topic->title }}</h3>
                                    <div class="text-left text-sm text-opacity-80 w-full two-line-text mb-2" style="color: {{ $block->subtitle_color }};">{{ strip_tags(html_entity_decode($topic->description)) }}</div>
