...
@endphp
<header id="main-header" style="{!! env('APP_NAME') == 'AABB-SP' ? 'position: absolute;':'position: fixed;' !!} top: 0; width: 100%; z-index: 1000;{!! env('APP_NAME') == 'AABB-SP' ? 'background: linear-gradient(to bottom, #010E2E 0%, transparent 100%);':'' !!}">
    @if(env('APP_NAME') == 'PróLab')
        @include('site.inc.prolab.alert')
    @endif
    <div class="container px-4 mx-auto">
        <nav class="flex items-center py-4">
            <a id="header-logo" style="{!! env('APP_NAME') == 'AABB-SP' ? 'margin-bottom: -6rem;':'' !!}" class="text-3xl font-semibold leading-none {!! env('APP_NAME') == 'AABB-SP' ? 'hidden md:block':'' !!}" href="/">
                <img id="main-logo" style="{!! env('APP_NAME') == 'AABB-SP' ? 'height: 8rem':(env('APP_NAME') == 'Estoque Legal' ? 'height: 3.5rem':'height: 3rem') !!}" src="{{ asset(str_replace('public/', 'storage/', $menu->logo)) }}" alt="{{ env('APP_NAME') }}" width="auto">
            </a>
            <div class="lg:hidden ml-auto">
                <button class="navbar-burger flex items-center py-2 px-3 border rounded menu-item-mobile" style="color: {{ $menu->item_color_scroll }}; border-color: {{ $menu->item_color_scroll }}">
                    ...
                    @if ($item_menu->type != 'menu')
                        <li>
                            @if(!$item_menu->background)
                                <a class="text-sm font-bold menu-item py-2" href="{{ $item_menu->type == 'link' ? $item_menu->link : '/' . ($item_menu->page->prefix_slug->count() > 0 ? implode('/', $item_menu->page->prefix_slug->pluck('slug')->toArray()) . '/' . $item_menu->page->slug : $item_menu->page->slug) }}" style="color: {{ $menu->item_color }}">{{ $item_menu->title }}</a>
                            @else
                    ...
                    @else
                        <li class="relative">
                            <span class="text-sm font-bold menu-item inline-block cursor-pointer py-2"
                                  ...
                                  style="padding-top: 10px; padding-bottom: 10px; color: {{ $menu->item_color }}">
                                {{ $item_menu->title }}
                            </span>
                            <div class="submenu absolute hidden shadow rounded-lg" style="top: 80%; left: -5%; background-color: {{ $menu->background_color_dropdown }}; width: {{ env('APP_NAME') == 'PróLab' ? '11':'10' }}rem">
                                @include('site.inc.headers.layout_2.partials.dropdown', ['submenus' => $item_menu->itemsMenu->sortBy('display_order')])
                            </div>
                        </li>
                    @endif
                @endforeach
        ...
        <nav class="relative flex flex-col py-6 px-6 w-full h-full overflow-y-auto" style="background-color: {{ $menu->background_color }}">
            <div class="flex items-center mb-8">
                <a class="mr-auto text-3xl font-semibold leading-none" href="/">
                    <img {!! env('APP_NAME') == 'AABB-SP' ? 'class="h-13"':'style="height: 2rem"' !!} src="{{ asset(str_replace('public/', 'storage/', $menu->logo)) }}" alt="{{ env('APP_NAME') }}" width="auto">
                </a>
                ...
                <ul>
                    @foreach ($items_menu as $item_menu)
                        @if ($item_menu->type != 'menu' || ($item_menu->menu_with_link && $item_menu->menu_with_link_mobile))
                            <li class="mb-1">
                                @if(!$item_menu->background)
                                    <a class="block p-4 text-sm font-bold menu-item-mobile" href="{{ $item_menu->type == 'link' || $item_menu->menu_with_link_type == 'link' ? $item_menu->link : '/' . ($item_menu->page->prefix_slug->count() > 0 ? implode('/', $item_menu->page->prefix_slug->pluck('slug')->toArray()) . '/' . $item_menu->page->slug : $item_menu->page->slug) }}" style="color: {{ $menu->item_color_scroll }}">{{ $item_menu->title }}</a>
                                @else
