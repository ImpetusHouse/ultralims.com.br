<style>
    .header-logo img {
        transition: opacity 0.3s ease;
    }

    .logo-fade-out {
        opacity: 0;
    }

    .logo-fade-in {
        opacity: 1;
    }

    #main-header {
        transition: background-color 0.3s ease;
    }

    .header-background {
        background-color: {{ $menu->background_color }};
    }

    .menu-item-color-scroll {
        color: {{ $menu->item_color_scroll }}  !important;
    }

    .menu-item-color-scroll:hover {
        color: {{ $menu->item_hover_color_scroll }}  !important;
    }

    .menu-item:hover {
        color: {{ $menu->item_hover_color }} !important;
        border-color: {{ $menu->item_hover_color }} !important;
    }

    .menu-item:hover > svg {
        color: {{ $menu->item_hover_color }} !important;
        border-color: {{ $menu->item_hover_color }} !important;
    }

    .menu-item-mobile:hover {
        color: {{ $menu->item_hover_color_scroll }} !important;
        border-color: {{ $menu->item_hover_color_scroll }} !important;
    }

    .menu-item-dropdown:hover {
        color: {{ $menu->item_hover_color_dropdown }} !important;
        border-color: {{ $menu->item_hover_color_dropdown }} !important;
    }

    .menu-bg:hover {
        background-color: {{ str_replace(')', ', 0.5)', $menu->background_color_dropdown) }};
    }

    .menu-arrow {
        transition: transform 0.3s ease;
    }

    .menu-arrow.rotate {
        transform: rotate(180deg);
    }

    .submenu {
        display: none;
        position: absolute;
    }

    .menu-item:hover + .submenu,
    .menu-item:focus + .submenu,
    .submenu:hover {
        display: block;
    }

    .menu-item-dropdown:hover + .submenu,
    .menu-item-dropdown:focus + .submenu,
    .submenu:hover {
        display: block;
    }

    @media (max-width: 767px) {
        #main-header {
            background-color: {{ $menu->background_color }};
        }

        .header-logo img {
            opacity: 1;
        }

        .menu-item {
            color: {{ $menu->item_color_scroll }}  !important;
        }

        .menu-item:hover {
            color: {{ $menu->item_hover_color_scroll }}  !important;
        }
    }
</style>
@php
    if (isset($page)){
        if ($page->group){
            $group = $page->group;
        }else{
            $group = \App\Models\Settings\Group_Item_Menu::where('default', true)->first();
        }
    }else{
        $group = \App\Models\Settings\Group_Item_Menu::where('default', true)->first();
    }
    $items_menu = $group->items->where('item_menu_id', null);
@endphp
<header id="main-header" style="position: fixed; top: 0; width: 100%; z-index: 1000;">
    <div class="container mx-auto px-4 relative h-20 py-4">
        <div class="flex -mx-4 h-full items-center justify-between">
            <a class="header-logo hidden md:block absolute transform -translate-x-1/2 -translate-y-1/2" href="/" style="top: 50%; left: 50%">
                <img class="main-logo" src="{{ asset(str_replace('public/', 'storage/', $menu->logo)) }}" width="133px" alt="{{ env('APP_NAME') }}">
            </a>
            <div class="w-auto sm:w-1/2 xl:w-auto px-4">
                <a class="header-logo inline-block md:hidden" href="/">
                    <img class="main-logo" src="{{ asset(str_replace('public/', 'storage/', $menu->logo)) }}" width="133px" alt="{{ env('APP_NAME') }}">
                </a>
                <div class="hidden xl:flex items-center">
                    @foreach ($items_menu as $item_menu)
                        @if ($item_menu->type != 'menu')
                            @if(!$item_menu->background)
                                <a class="inline-flex lg:mr-6 xl:mr-10 items-center text-sm font-bold menu-item" href="{{ $item_menu->type == 'link' ? $item_menu->link : '/' . ($item_menu->page->prefix_slug->count() > 0 ? implode('/', $item_menu->page->prefix_slug->pluck('slug')->toArray()) . '/' . $item_menu->page->slug : $item_menu->page->slug) }}" style="color: {{ $menu->item_color }}">
                                    <span class="mr-2">{{ $item_menu->title }}</span>
                                </a>
                            @else
                                <a class="mr-2 inline-block px-4 py-3 text-xs font-semibold leading-none hover:opacity-70 rounded" href="{{ $item_menu->type == 'link' ? $item_menu->link : '/' . ($item_menu->page->prefix_slug->count() > 0 ? implode('/', $item_menu->page->prefix_slug->pluck('slug')->toArray()) . '/' . $item_menu->page->slug : $item_menu->page->slug) }}" style="color: {{ $item_menu->title_color }}; background-color: {{ $item_menu->background_color }}">{{ $item_menu->title }}</a>
                            @endif
                        @else
                            <span class="relative cursor-pointer">
                                <span class="menu-item inline-flex lg:mr-6 xl:mr-10 items-center text-sm font-bold" style="color: {{ $menu->item_color }}"
                                      @if($item_menu->menu_with_link) onclick="location.href = '{{ $item_menu->menu_with_link_type == 'link' ? $item_menu->link : '/' . ($item_menu->page->prefix_slug->count() > 0 ? implode('/', $item_menu->page->prefix_slug->pluck('slug')->toArray()) . '/' . $item_menu->page->slug : $item_menu->page->slug) }}'" @endif>
                                    <span class="mr-2">{{ $item_menu->title }}</span>
                                    <svg width="16" height="16" viewbox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" style="color: {{ $menu->item_color }}">
                                        <path d="M8.47315 10.36L12.2398 6.58667C12.3023 6.52469 12.3519 6.45096 12.3857 6.36972C12.4196 6.28848 12.437 6.20134 12.437 6.11333C12.437 6.02533 12.4196 5.93819 12.3857 5.85695C12.3519 5.77571 12.3023 5.70198 12.2398 5.64C12.1149 5.51583 11.9459 5.44614 11.7698 5.44614C11.5937 5.44614 11.4247 5.51583 11.2998 5.64L7.96648 8.94L4.66648 5.64C4.54157 5.51583 4.3726 5.44614 4.19648 5.44614C4.02036 5.44614 3.85139 5.51583 3.72648 5.64C3.66349 5.70174 3.61337 5.77537 3.57904 5.85662C3.54471 5.93787 3.52685 6.02513 3.52648 6.11333C3.52685 6.20154 3.54471 6.28879 3.57904 6.37004C3.61337 6.45129 3.66349 6.52492 3.72648 6.58667L7.49315 10.36C7.55557 10.4277 7.63134 10.4817 7.71568 10.5186C7.80001 10.5556 7.89108 10.5746 7.98315 10.5746C8.07521 10.5746 8.16628 10.5556 8.25062 10.5186C8.33495 10.4817 8.41072 10.4277 8.47315 10.36Z" fill="currentColor"></path>
                                    </svg>
                                </span>
                                <div class="submenu absolute hidden shadow rounded-lg" style="background-color: {{ $menu->background_color_dropdown }}; width: {{ env('APP_NAME') == 'PróLab' ? '11':'10' }}rem">
                                    @include('site.inc.headers.layout_3.partials.dropdown', ['submenus' => $item_menu->itemsMenu->sortBy('display_order')])
                                </div>
                            </span>
                        @endif
                    @endforeach
                </div>
            </div>

            <div class="w-auto sm:w-1/2 xl:w-auto px-4">
                <div class="flex items-center justify-end">
                    <a class="hidden sm:inline-block mr-6 menu-item" href="#" style="color: {{ $menu->item_color }}">
                        <svg width="24" height="24" viewbox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M21.2913 4.61183C20.7805 4.10083 20.1741 3.69547 19.5066 3.41891C18.8392 3.14235 18.1238 3 17.4013 3C16.6788 3 15.9634 3.14235 15.2959 3.41891C14.6285 3.69547 14.022 4.10083 13.5113 4.61183L12.4513 5.67183L11.3913 4.61183C10.3596 3.58013 8.96032 3.00053 7.50129 3.00053C6.04226 3.00053 4.64298 3.58013 3.61129 4.61183C2.5796 5.64352 2 7.04279 2 8.50183C2 9.96086 2.5796 11.3601 3.61129 12.3918L4.67129 13.4518L12.4513 21.2318L20.2313 13.4518L21.2913 12.3918C21.8023 11.8811 22.2076 11.2746 22.4842 10.6072C22.7608 9.93972 22.9031 9.22431 22.9031 8.50183C22.9031 7.77934 22.7608 7.06393 22.4842 6.39647C22.2076 5.72901 21.8023 5.12258 21.2913 4.61183V4.61183Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
                        </svg>
                    </a>
                    <a class="hidden sm:inline-block mr-6 menu-item" href="{{ route('store.cart') }}" style="color: {{ $menu->item_color }}">
                        <svg width="24" height="24" viewbox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M9 23C9.55228 23 10 22.5523 10 22C10 21.4477 9.55228 21 9 21C8.44772 21 8 21.4477 8 22C8 22.5523 8.44772 23 9 23Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
                            <path d="M20 23C20.5523 23 21 22.5523 21 22C21 21.4477 20.5523 21 20 21C19.4477 21 19 21.4477 19 22C19 22.5523 19.4477 23 20 23Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
                            <path d="M1 2H5L7.68 15.39C7.77144 15.8504 8.02191 16.264 8.38755 16.5583C8.75318 16.8526 9.2107 17.009 9.68 17H19.4C19.8693 17.009 20.3268 16.8526 20.6925 16.5583C21.0581 16.264 21.3086 15.8504 21.4 15.39L23 7H6" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
                        </svg>
                    </a>
                    <a class="hidden sm:inline-block mr-6 menu-item" href="{{ route('store.search') }}" style="color: {{ $menu->item_color }}">
                        <svg width="20" height="20" viewbox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M9 17C13.4183 17 17 13.4183 17 9C17 4.58172 13.4183 1 9 1C4.58172 1 1 4.58172 1 9C1 13.4183 4.58172 17 9 17Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
                            <path d="M18.9999 19L14.6499 14.65" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
                        </svg>
                    </a>
                    <a class="hidden sm:inline-block menu-item" href="{{ route('store.history') }}" style="color: {{ $menu->item_color }}">
                        <svg width="24" height="24" viewbox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M20 20V18C20 16.9391 19.5786 15.9217 18.8284 15.1716C18.0783 14.4214 17.0609 14 16 14H8C6.93913 14 5.92172 14.4214 5.17157 15.1716C4.42143 15.9217 4 16.9391 4 18V20" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
                            <path d="M12 10C14.2091 10 16 8.20914 16 6C16 3.79086 14.2091 2 12 2C9.79086 2 8 3.79086 8 6C8 8.20914 9.79086 10 12 10Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
                        </svg>
                    </a>
                    <button class="xl:hidden ml-6 navbar-burger menu-item" style="color: {{ $menu->item_color }}">
                        <svg width="24" height="24" viewbox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M3 12H21" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
                            <path d="M3 6H21" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
                            <path d="M3 18H21" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
                        </svg>
                    </button>
                </div>
            </div>
        </div>
    </div>

    <div class="hidden navbar-menu fixed top-0 left-0 bottom-0 w-5/6 max-w-md z-50">
        <div class="navbar-backdrop fixed inset-0 bg-blueGray-800 opacity-70"></div>
        <nav class="relative flex flex-col pt-12 pb-6 px-8 w-full h-full overflow-y-auto" style="background-color: {{ $menu->background_color }}">
            <div class="flex mb-12 items-center">
                <a class="inline-block mr-auto" href="/">
                    <img class="h-8" src="{{ asset(str_replace('public/', 'storage/', $menu->logo)) }}" alt="{{ env('APP_NAME') }}">
                </a>
                <button class="navbar-close menu-item" style="color: {{ $menu->item_color_scroll }}">
                    <svg class="h-6 w-6 cursor-pointer" xmlns="http://www.w3.org/2000/svg" fill="none" viewbox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                    </svg>
                </button>
            </div>
            <div class="flex w-full max-w-xs items-center px-6 border border-coolGray-200 rounded-full">
                <input class="h-12 w-full bg-transparent border-0 text-sm text-gray-500 placeholder-gray-500 outline-none" type="search" placeholder="Pesquisar...">
                <button class="inline-block ml-auto text-gray-400 hover:text-rhino-500" type="submit">
                    <svg width="14" height="14" viewbox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M6.33333 11.6667C9.27885 11.6667 11.6667 9.27885 11.6667 6.33333C11.6667 3.38782 9.27885 1 6.33333 1C3.38782 1 1 3.38782 1 6.33333C1 9.27885 3.38782 11.6667 6.33333 11.6667Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
                        <path d="M13.0001 13L10.1001 10.1" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
                    </svg>
                </button>
            </div>
            <div class="py-12 mb-auto">
                <ul class="flex-col">
                    <li class="mb-3">
                        <a class="group mr-6 inline-flex items-center text-base" style="color: {{ env('PRIMARY_COLOR') }}" href="#">
                            <span class="mr-2">
                              <svg width="16" height="16" viewbox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M13.3334 13.3333V12C13.3334 11.2928 13.0525 10.6145 12.5524 10.1144C12.0523 9.61429 11.374 9.33334 10.6667 9.33334H5.33341C4.62617 9.33334 3.94789 9.61429 3.4478 10.1144C2.9477 10.6145 2.66675 11.2928 2.66675 12V13.3333" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
                                <path d="M7.99992 6.66667C9.47268 6.66667 10.6666 5.47276 10.6666 4C10.6666 2.52724 9.47268 1.33334 7.99992 1.33334C6.52716 1.33334 5.33325 2.52724 5.33325 4C5.33325 5.47276 6.52716 6.66667 7.99992 6.66667Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
                              </svg>
                            </span>
                            <span class="font-semibold" style="color: {{ $menu->item_color_scroll }}">Login</span>
                        </a>
                    </li>
                    <li class="mb-3">
                        <a class="group mr-6 inline-flex items-center text-base" style="color: {{ env('PRIMARY_COLOR') }}" href="#">
                            <span class="mr-2">
                              <svg width="16" height="16" viewbox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <g clip-path="url(#clip0_1208_8162)">
                                  <path d="M14.1941 3.07455C13.8536 2.73389 13.4493 2.46365 13.0043 2.27927C12.5594 2.0949 12.0824 2 11.6008 2C11.1191 2 10.6422 2.0949 10.1972 2.27927C9.75224 2.46365 9.34795 2.73389 9.00745 3.07455L8.30078 3.78122L7.59411 3.07455C6.90632 2.38676 5.97347 2.00036 5.00078 2.00036C4.02809 2.00036 3.09524 2.38676 2.40745 3.07455C1.71965 3.76235 1.33325 4.69519 1.33325 5.66788C1.33325 6.64057 1.71965 7.57342 2.40745 8.26122L3.11411 8.96788L8.30078 14.1545L13.4874 8.96788L14.1941 8.26122C14.5348 7.92071 14.805 7.51643 14.9894 7.07145C15.1738 6.62648 15.2687 6.14954 15.2687 5.66788C15.2687 5.18623 15.1738 4.70929 14.9894 4.26431C14.805 3.81934 14.5348 3.41505 14.1941 3.07455V3.07455Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
                                </g>
                              </svg>
                            </span>
                            <span class="font-semibold" style="color: {{ $menu->item_color_scroll }}">Favorite</span>
                        </a>
                    </li>
                    <li class="mb-12">
                        <a class="inline-flex items-center text-base" style="color: {{ env('PRIMARY_COLOR') }}" href="#">
                            <span class="mr-2">
                              <svg width="18" height="17" viewbox="0 0 18 17" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M6.99992 15.3333C7.36811 15.3333 7.66658 15.0349 7.66658 14.6667C7.66658 14.2985 7.36811 14 6.99992 14C6.63173 14 6.33325 14.2985 6.33325 14.6667C6.33325 15.0349 6.63173 15.3333 6.99992 15.3333Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
                                <path d="M14.3334 15.3333C14.7016 15.3333 15.0001 15.0349 15.0001 14.6667C15.0001 14.2985 14.7016 14 14.3334 14C13.9652 14 13.6667 14.2985 13.6667 14.6667C13.6667 15.0349 13.9652 15.3333 14.3334 15.3333Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
                                <path d="M1.66675 1.33334H4.33341L6.12008 10.26C6.18104 10.5669 6.34802 10.8426 6.59178 11.0389C6.83554 11.2351 7.14055 11.3393 7.45341 11.3333H13.9334C14.2463 11.3393 14.5513 11.2351 14.7951 11.0389C15.0388 10.8426 15.2058 10.5669 15.2667 10.26L16.3334 4.66667H5.00008" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
                              </svg>
                            </span>
                            <span class="font-semibold" style="color: {{ $menu->item_color_scroll }}">Cart</span>
                        </a>
                    </li>

                    @foreach ($items_menu as $item_menu)
                        @if ($item_menu->type != 'menu' || ($item_menu->menu_with_link && $item_menu->menu_with_link_mobile))
                            <li class="mb-4">
                                @if(!$item_menu->background)
                                    <a class="block text-sm font-bold menu-item-mobile" href="{{ $item_menu->type == 'link' || $item_menu->menu_with_link_type == 'link' ? $item_menu->link : '/' . ($item_menu->page->prefix_slug->count() > 0 ? implode('/', $item_menu->page->prefix_slug->pluck('slug')->toArray()) . '/' . $item_menu->page->slug : $item_menu->page->slug) }}" style="color: {{ $menu->item_color_scroll }}">{{ $item_menu->title }}</a>
                                @else
                                    <a class="block px-4 py-3 mb-3 text-xs text-center font-semibold leading-none hover:opacity-70 rounded" href="{{ $item_menu->type == 'link' || $item_menu->menu_with_link_type == 'link' ? $item_menu->link : '/' . ($item_menu->page->prefix_slug->count() > 0 ? implode('/', $item_menu->page->prefix_slug->pluck('slug')->toArray()) . '/' . $item_menu->page->slug : $item_menu->page->slug) }}" style="color: {{ $item_menu->title_color }}; background-color: {{ $item_menu->background_color }}">{{ $item_menu->title }}</a>
                                @endif
                            </li>
                        @else
                            <span class="flex justify-between items-center text-sm font-semibold cursor-pointer menu-item-mobile mb-4" style="color: {{ $menu->item_color_scroll }}" onclick="$(this).next().slideToggle(); $(this).children('.menu-arrow').toggleClass('rotate')">
                                {{ $item_menu->title }}
                                <svg class="menu-arrow h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                                </svg>
                            </span>
                            @include('site.inc.headers.layout_2.partials.submenu', ['submenus' => $item_menu->itemsMenu->sortBy('display_order')])
                        @endif
                    @endforeach
                </ul>
            </div>
        </nav>
    </div>
</header>

@push('scripts')
    <script>
        // Burger menus
        document.addEventListener('DOMContentLoaded', function() {
            // open
            const burger = document.querySelectorAll('.navbar-burger');
            const menu = document.querySelectorAll('.navbar-menu');
            if (burger.length && menu.length) {
                for (var i = 0; i < burger.length; i++) {
                    burger[i].addEventListener('click', function() {
                        for (var j = 0; j < menu.length; j++) {
                            menu[j].classList.toggle('hidden');
                        }
                    });
                }
            }

            // close
            const close = document.querySelectorAll('.navbar-close');
            const backdrop = document.querySelectorAll('.navbar-backdrop');
            if (close.length) {
                for (var i = 0; i < close.length; i++) {
                    close[i].addEventListener('click', function() {
                        for (var j = 0; j < menu.length; j++) {
                            menu[j].classList.toggle('hidden');
                        }
                    });
                }
            }
            if (backdrop.length) {
                for (var i = 0; i < backdrop.length; i++) {
                    backdrop[i].addEventListener('click', function() {
                        for (var j = 0; j < menu.length; j++) {
                            menu[j].classList.toggle('hidden');
                        }
                    });
                }
            }

            const searchButton = document.querySelectorAll('.searchButton');
            const searchInput = document.querySelectorAll('.animate-searchInput');
            if (searchButton.length && searchInput.length) {
                for (var i = 0; i < searchButton.length; i++) {
                    searchButton[i].addEventListener('click', function() {
                        for (var j = 0; j < searchInput.length; j++) {
                            searchInput[j].classList.remove('hidden');
                        }
                    });
                }
            }

            // menus
            const header = document.getElementById('main-header');
            const logo = document.getElementsByClassName('main-logo');
            const menuItems = document.querySelectorAll('.menu-item');
            const appName = '{!! env('APP_NAME') !!}';
            const logoScroll = "{{ asset(str_replace('public/', 'storage/', $menu->logo_scroll)) }}";
            const originalLogo = "{{ asset(str_replace('public/', 'storage/', $menu->logo)) }}";

            let isLogoChanged = false;

            function handleScroll() {
                if (window.scrollY > 50 && appName !== 'AABB-SP') {
                    header.classList.add('header-background');
                    if (!isLogoChanged) {
                        fadeOutAndChangeLogo(logo, logoScroll);
                        isLogoChanged = true;
                    }
                    menuItems.forEach(item => {
                        item.classList.add('menu-item-color-scroll');
                    });
                } else {
                    header.classList.remove('header-background');
                    if (isLogoChanged) {
                        fadeOutAndChangeLogo(logo, originalLogo);
                        isLogoChanged = false;
                    }
                    menuItems.forEach(item => {
                        item.classList.remove('menu-item-color-scroll');
                    });
                }
            }
            function fadeOutAndChangeLogo(logoElement, newSrc) {
                // Converte HTMLCollection para Array
                const logosArray = Array.from(logoElement);
                logosArray.forEach((element) => {
                    element.classList.add('logo-fade-out');
                    element.classList.remove('logo-fade-in');
                    setTimeout(() => {
                        element.src = newSrc;
                        element.classList.add('logo-fade-in');
                        element.classList.remove('logo-fade-out');
                    }, 0);
                });
            }

            window.addEventListener('scroll', handleScroll);
            handleScroll();
        });
    </script>
@endpush
