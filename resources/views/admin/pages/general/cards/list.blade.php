...
@section('content')
        ...
        <div class="card-header align-items-center py-5 gap-2 gap-md-5">
            ...
            <div class="card-title">
                ...
                <div class="d-flex align-items-center position-relative my-1">
                    ...
                    <!--end::Svg Icon-->
                    <input type="text" data-kt-ecommerce-product-filter="search" class="form-control form-control-solid w-250px ps-14" placeholder="Procurar card"/>
                </div>
                <!--end::Search-->
            </div>
            <!--end::Card title-->
            ...
            <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                <div class="w-100 mw-150px">
                    <!--begin::Select2-->
                    <select class="form-select form-select-solid" data-control="select2" data-hide-search="true" data-placeholder="Categoria" data-kt-ecommerce-product-filter="category">
                        <option></option>
                ...
                <!--begin::Add product-->
                <button type="button" data-bs-toggle="modal" data-bs-target="#modalOrderCards" class="btn btn-primary">Ordenar</button>
                <!--end::Add product-->
                <!--begin::Add product-->
                <a href="{{ route('admin.cards.create') }}" class="btn btn-primary">Adicionar card</a>
                <!--end::Add product-->
            </div>
            <!--end::Card toolbar-->
        </div>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="d-flex align-items-center">
                                <div>
                                    <!--begin::Title-->
                                    <a href="{{ route('admin.cards.edit', $card->hash_id) }}" class="text-gray-800 text-hover-primary fs-5 fw-bold" data-kt-ecommerce-product-filter="name">
                                        {{ $card->title }}
                                    </a>
                                    <a class="d-none" data-kt-ecommerce-product-filter="id">{{ $card->hash_id }}</a>
                                    <!--end::Title-->
                                </div>
                            </div>
                        </td>
                        <td data-order="{{ $card->category->title }}">{{$card->category->title}}</td>
                        <td data-order="{{ $card->subcategory ? $card->subcategory->title:'' }}">{{$card->subcategory ? $card->subcategory->title:'' }}</td>
                        <td>{{ Str::limit(trim(strip_tags(html_entity_decode($card->description))), 50, '...') }}</td>
                        @php
                            $status = '';
                            $badge = '';
                            /** @var TYPE_NAME $card */
                        ...
                        <td class="text-center">
                            ...
                            <div
                                ...
                                <div class="menu-item px-3">
                                    <a href="{{route('admin.cards.edit', $card->hash_id)}}" class="menu-link px-3">Editar</a>
                                </div>
                                <!--end::Menu item-->
                                <!--begin::Menu item-->
                                <div class="menu-item px-3">
    ...
    <div class="modal fade" id="modalOrderCards" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
...
@push('scripts')
            ...
            var t, e, n = () => {
                t.querySelectorAll('[data-kt-ecommerce-product-filter="delete_row"]').forEach((t => {
                    t.addEventListener("click", (function (t) {
                        ...
                        Swal.fire({
                            text: "Tem certeza que deseja excluir o card?",
                            icon: "question",
                            showCancelButton: !0,
                            buttonsStyling: !1,
                            ...
                            if(t.value){
                                $.ajax({
                                    method: "DELETE",
                                    url: `{{route('admin.cards.destroy', '_id_')}}`.replace('_id_', n.querySelector('[data-kt-ecommerce-product-filter="id"]').innerText),
                                    success: function (rtn) {
                                        if (rtn.success === true) {
                                            Swal.fire({
                                                text: "Você excluiu o card",
                                                icon: "success",
                                                buttonsStyling: !1,
            ...
            return {
                init: function () {
                    (t = document.querySelector("#kt_ecommerce_products_table")) && ((e = $(t).DataTable({
                        ...
                        const t = document.querySelector('[data-kt-ecommerce-product-filter="category"]');
                        $(t).on("change", (t => {
                            let n = t.target.value;
                            "todos" === n && (n = ""), e.column(1).search(n).draw()
                        }))
                    })(), n())
                }
            }
    ...
    <script>
        ...
        $('#modalOrderCards').on('hide.bs.modal', function (e) {
        ...
        $('#order-type').on('change', function (){
            ...
            $.ajax({
                type: 'GET',
                url: '{{route('admin.cards.order', '_TYPE_')}}'.replace('_TYPE_', this.value),
            }).then(function (data) {
                if (data.success) {
                    ...
                    }else if (orderType === 'subcategory') {
                        $('#order-type-title').html('Categoria');
                    } else if (orderType === 'card') {
                        $('#order-type-title').html('Subcategoria');
                    }
                    ...
                    $('#order-type-value').select2({
                        dropdownParent: $('#modalOrderCards .modal-body') // Ajuste o seletor conforme necessário
                    });
                }else{
        ...
        $('#order-type-value').on('change', function (){
            $.ajax({
                type: 'GET',
                url: '{{route('admin.cards.order.type', ['type' => '_TYPE_', 'id' => '_ID_'])}}'.replace('_TYPE_', $('#order-type').val()).replace('_ID_', this.value),
            }).then(function (data) {
                if (data.success) {
                    var html;
        ...
        $('#form-order').on('submit', function (e){
           e.preventDefault();
            ...
            $.ajax({
                url: '{{route('admin.cards.order.type', ['type' => '_TYPE_', 'id' => '_ID_'])}}'.replace('_TYPE_', $('#order-type').val()).replace('_ID_', $('#order-type-value').val()),
                type: 'POST',
                data: form,
                processData: false,
                contentType: false,
