...
@section('content')
    <!--begin::Form-->
    <form class="form d-flex flex-column flex-lg-row saveProduct">
        <!--begin::Aside column-->
            ...
            <div class="card card-flush py-4">
                ...
                <div class="card-body pt-0">
                    ...
                    <select class="form-select mb-2" data-control="select2" data-hide-search="true" data-placeholder="Selecione uma opção" id="status" name="status">
                        <option></option>
                        <option value="1" {{ $product->status == 1 || $product->id == null ? 'selected':'' }}>Ativo</option>
                        <option value="0" {{ $product->status == 0 && $product->id > 0 ? 'selected':'' }}>Inativo</option>
                    </select>
                    <!--end::Select2-->
                    <!--begin::Description-->
                    <div class="text-muted fs-7">Defina o status do produto.</div>
                    <!--end::Description-->
                </div>
                <!--end::Card body-->
            </div>
                ...
                <div class="card-body pt-0">
                    ...
                    <select class="form-select mb-2 categorySelect" name="categories[]" data-control="select2" data-placeholder="Selecione uma opção" multiple="multiple">
                        ...
                        @foreach ($categories as $category)
                            <option value="{{$category->id}}" {{ $product->categories->pluck('id')->contains($category->id) ? 'selected':'' }}>{{$category->title}}</option>
                        @endforeach
                    </select>
                    <!--end::Select2-->
                    <!--begin::Description-->
            ...
            <div class="card card-flush py-4">
                ...
                <div class="card-body pt-0">
                    ...
                    <div class="mb-10 fv-row">
                        ...
                        <!--end::Label-->
                        <!--begin::Input-->
                        <input type="text" name="title" class="form-control mb-2" value="{{ $product->title }}"/>
                        <!--end::Input-->
                        <!--begin::Description-->
                    ...
                    <div class="mb-10 row">
                        <div class="col-12">
                            ...
                            <!--end::Label-->
                            <!--begin::Input-->
                            <input type="text" name="slug" class="form-control mb-2" value="{{ $product->slug }}"/>
                            <!--end::Input-->
                            <!--begin::Description-->
                            <div class="text-muted fs-7">
                                @if($product->id > 0)
                                    <a href="{{url('/')}}/{{ env('APP_NAME') == 'Ultra Lims' ? 'cliente-ultra/extensoes-lims':'produtos' }}/{{ $product->slug }}" target="_blank">
                                        {{url('/')}}/<span id="preview-type">{{ env('APP_NAME') == 'Ultra Lims' ? 'cliente-ultra/extensoes-lims':'produtos' }}/</span><span id="preview-slug">{{ $product->slug }}</span>
                                    </a>
                                @else
                                    {{url('/')}}/<span id="preview-type">{{ env('APP_NAME') == 'Ultra Lims' ? 'cliente-ultra/extensoes-lims':'produtos' }}/</span><span id="preview-slug">{{ $product->slug }}</span>
                                @endif
                    ...
                    <div class="mb-10 fv-row">
                        ...
                        <!--end::Label-->
                        <!--begin::Input-->
                        <textarea type="text" id="description" class="form-control mb-2" rows="4" style="resize: none">{!! nl2br($product->description) !!}</textarea>
                        <!--end::Input-->
                        <!--begin::Description-->
                        <div class="text-muted fs-7">Uma descrição é obrigatória.</div>
                        <!--end::Description-->
                    </div>
                    <!--end::Input group-->
                    <!--begin::Input group-->
                    <div class="fv-row">
                        <!--begin::Label-->
                        <label class="required form-label">Vantagens</label>
                        <!--end::Label-->
                        <!--begin::Input-->
                        <textarea type="text" id="benefits" class="form-control mb-2" rows="4" style="resize: none">{!! nl2br($product->benefits) !!}</textarea>
                        <!--end::Input-->
                    </div>
                    <!--end::Input group-->
                </div>
...
@push('scripts')
    ...
    <script>
        @if($product->photo != null)
            let user_profile = '{{ str_replace('public/', 'storage/', asset($product->photo)) }}';
    ...
    <script>
        $('.saveProduct').on("submit", function(e){
            ...
            form.append('benefits', benefits.getData());
            @if($product->id > 0)
            ...
            $.ajax({
                url: '{{$product->id > 0 ? route('admin.products.update', $product->hash_id):route('admin.products.store')}}',
                type: 'POST',
                data: form,
                processData: false,
                contentType: false,
                ...
                error: function(data){
                    ...
                    Swal.fire('Falha!', 'Falha ao {{ $product->id > 0 ? 'salvar':'editar' }} produto, tente novamente', 'error');
                }
            });
    ...
    <script>
        ...
        function generateSlug(title){
            ...
            $.ajax({
                url: '{{route('admin.products.get.slug')}}',
                type: 'POST',
                data: {title: title},
                dataType: 'json',
                success: function(data){
        ...
        $('#saveCategory').on("click", function(e){
            ...
            if(val.trim() != ''){
                if(ModalCategoryTypeSend == 'store'){
                    ...
                    $.ajax({
                        type: "PUT",
                        url: "{{route('admin.products.categories.update', '_id_')}}".replace('_id_', ModalCategoryChange),
        ...
        function deleteCategory(category_id){
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        method: "DELETE",
                        url: "{{route('admin.products.categories.destroy', '_id_')}}".replace('_id_', category_id),
                        data: {id: category_id},
                        success: function(rtn){
                            if(rtn.success){
                                if(rtn.reloaded){
