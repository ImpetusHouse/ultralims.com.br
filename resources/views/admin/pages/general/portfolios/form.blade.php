            ...
            <div class="card card-flush py-4">
                ...
                <div class="card-body pt-0">
                    ...
                    <select class="form-select mb-2" data-control="select2" data-hide-search="true" data-placeholder="Selecione uma opção" id="status" name="status">
                        <option></option>
                        <option value="1" {{ $portfolio->status == 1 || $portfolio->id == null ? 'selected':'' }}>Ativo</option>
                        <option value="0" {{ $portfolio->status == 0 && $portfolio->id > 0 ? 'selected':'' }}>Inativo</option>
                    </select>
                    <!--end::Select2-->
                    <!--begin::Description-->
                ...
                <div class="card-body pt-0">
                    ...
                    <select class="form-select mb-2 categorySelect" name="categories[]" data-control="select2" data-placeholder="Selecione uma opção" multiple="multiple">
                        ...
                        @foreach ($categories as $category)
                            <option value="{{$category->id}}" {{ $portfolio->categories->pluck('id')->contains($category->id) ? 'selected':'' }}>{{$category->title}}</option>
                        @endforeach
                    </select>
                    <!--end::Select2-->
                    <!--begin::Description-->
                    <div class="text-muted fs-7 mb-7">Adicionar página a um categoria.</div>
                    <!--end::Description-->
                    <!--end::Input group-->
                    <!--begin::Button-->
                    <a href="#" data-bs-toggle="modal" data-bs-target="#makeCategoriesModal" class="btn btn-light-primary btn-sm mb-8">
                    ...
                    <!--end::Label-->
                    <!--begin::Select2-->
                    <select class="form-select mb-2" name="page_id" data-control="select2" data-placeholder="Selecione uma opção">
                        <option></option>
                        @foreach ($pages as $page)
                            <option value="{{$page->id}}" {{ $page->id == $portfolio->page_id ? 'selected':'' }}>{{$page->title}}</option>
                        @endforeach
                    </select>
                    <!--end::Select2-->
                    <!--begin::Description-->
                    <div class="text-muted fs-7">Uma página é obrigatória.</div>
                    <!--end::Description-->
                </div>
                <!--end::Card body-->
            </div>
            ...
            <div class="card card-flush py-4">
                ...
                <div class="card-body pt-0">
                    ...
                    <div class="mb-10 fv-row">
                        ...
                        <!--end::Label-->
                        <!--begin::Input-->
                        <input type="text" name="title" class="form-control mb-2" value="{{ $portfolio->title }}"/>
                        <!--end::Input-->
                        <!--begin::Description-->
                    ...
                    <div class="mb-10 fv-row">
                        ...
                        <!--end::Label-->
                        <!--begin::Input-->
                        <input type="text" name="tag" class="form-control mb-2" value="{{ $portfolio->tag }}"/>
                        <!--end::Input-->
                    </div>
                    <!--end::Input group-->
                    <!--begin::Input group-->
                    <div class="fv-row">
                        ...
                        <!--end::Label-->
                        <!--begin::Input-->
                        <textarea type="text" name="description" class="form-control mb-2" rows="4" style="resize: none">{!! nl2br($portfolio->description) !!}</textarea>
                        <!--end::Input-->
                        <!--begin::Description-->
                        <div class="text-muted fs-7">Um descrição é obrigatória.</div>
                        <!--end::Description-->
                    </div>
                    <!--end::Input group-->
                </div>
...
@push('scripts')
    ...
    <script>
        @if($portfolio->photo != null)
            let user_profile = '{{ str_replace('public/', 'storage/', asset($portfolio->photo)) }}';
    ...
    <script>
        $('.saveArticle').on("submit", function(e){
            ...
            $.ajax({
                url: '{{$portfolio->id > 0 ? route('admin.portfolios.update', $portfolio->hash_id):route('admin.portfolios.store')}}',
                type: 'POST',
                data: form,
                processData: false,
                contentType: false,
                success: function(data){
                    ...
                    if(data.success){
                        Swal.fire({
                            ...
                            showCancelButton: !0,
                            buttonsStyling: !1,
                            confirmButtonText: "Ver portfólios",
                            cancelButtonText: "Novo portfólio",
                            customClass: {
                            ...
                            if (t.value){
                                ...
                                window.location.href = '{{route('admin.portfolios.create')}}'
                            }
                        }))
                    }else{
                ...
                error: function(data){
                    ...
                    Swal.fire('Falha!', 'Falha ao {{ $portfolio->id > 0 ? 'salvar':'editar' }} portfólio, tente novamente', 'error');
                }
            });
    ...
    <script>
        ...
        $('.cropped-image-send').on('click', function(e){
            ...
            $croppie.croppie('result', {
                type: 'canvas',
                size: {width: 1000, height: 1000}, // Dobro da largura e altura do viewport
                quality: 1
            }).then(function (resp) {
                $croppedImage = resp;
    ...
    <script>
        ...
        $('#saveCategory').on("click", function(e){
            ...
            if(val.trim() != ''){
                if(ModalCategoryTypeSend == 'store'){
                    $.ajax({
                        method: "POST",
                        url: "{{route('admin.portfolios.categories.store')}}",
                        data: {name: val},
                    ...
                    $.ajax({
                        type: "PUT",
                        url: "{{route('admin.portfolios.categories.update', '_id_')}}".replace('_id_', ModalCategoryChange),
        ...
        function deleteCategory(category_id){
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        method: "DELETE",
                        url: "{{route('admin.portfolios.categories.destroy', '_id_')}}".replace('_id_', category_id),
                        data: {id: category_id},
                        success: function(rtn){
                            if(rtn.success){
                                if(rtn.reloaded){
        ...
        function fetchSelect2(){
            ...
            $.ajax({
                type: 'GET',
                data: {ajaxJson: true},
                url: '{{route('admin.portfolios.categories.index')}}',
            }).then(function (data) {
