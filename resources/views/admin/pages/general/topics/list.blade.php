        ...
        <div class="card-header align-items-center py-5 gap-2 gap-md-5">
            ...
            <div class="card-title">
                ...
                <div class="d-flex align-items-center position-relative my-1">
                    ...
                    <!--end::Svg Icon-->
                    <input type="text" data-kt-ecommerce-product-filter="search" class="form-control form-control-solid w-250px ps-14" placeholder="Procurar tópico"/>
                </div>
                <!--end::Search-->
            </div>
            <!--end::Card title-->
            ...
            <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                ...
                <!--begin::Add product-->
                <a href="{{ route('admin.topics.create') }}" class="btn btn-primary">Adicionar tópico</a>
                <!--end::Add product-->
            </div>
            <!--end::Card toolbar-->
        </div>
                ...
                @foreach ($topics as $topic)
                    <tr>
                        ...
                        <td>
                            ...
                            <a class="d-none" data-kt-ecommerce-product-filter="id">{{ $topic->hash_id }}</a>
                        </td>
                        <td>{{ Str::limit(trim(strip_tags(html_entity_decode($topic->title))), 50, '...') }}</td>
                        <td>{{ Str::limit(trim(strip_tags(html_entity_decode($topic->description))), 60, '...') }}</td>
                        <td class="text-center">{{ $topic->category->display == '1' ? 'Sim':'Não' }}</td>
                        @php
                            $status = '';
                            $badge = '';
                            /** @var TYPE_NAME $topic */
                        ...
                        <td class="text-center">
                            ...
                            <div
                                ...
                                <div class="menu-item px-3">
                                    <a href="{{route('admin.topics.edit', $topic->hash_id)}}" class="menu-link px-3">Editar</a>
                                </div>
                                <!--end::Menu item-->
                                <!--begin::Menu item-->
                                <div class="menu-item px-3">
            ...
            var t, e, n = () => {
                t.querySelectorAll('[data-kt-ecommerce-product-filter="delete_row"]').forEach((t => {
                    t.addEventListener("click", (function (t) {
                        ...
                        Swal.fire({
                            text: "Tem certeza que deseja excluir o tópico?",
                            icon: "question",
                            showCancelButton: !0,
                            buttonsStyling: !1,
                            ...
                            if(t.value){
                                $.ajax({
                                    method: "DELETE",
                                    url: `{{route('admin.topics.destroy', '_id_')}}`.replace('_id_', n.querySelector('[data-kt-ecommerce-product-filter="id"]').innerText),
                                    success: function (rtn) {
                                        if (rtn.success === true) {
                                            Swal.fire({
                                                text: "Você excluiu o tópico!.",
                                                icon: "success",
                                                buttonsStyling: !1,
