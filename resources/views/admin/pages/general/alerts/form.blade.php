            ...
            <div class="card card-flush py-4">
                ...
                <div class="card-body pt-0">
                    ...
                    <select class="form-select mb-2" data-control="select2" data-hide-search="true" data-placeholder="Selecione uma opção" id="status" name="status">
                        <option></option>
                        <option value="1" {{ $alert->status == 1 || $alert->id == null ? 'selected':'' }}>Ativo</option>
                        <option value="0" {{ $alert->status == 0 && $alert->id > 0 ? 'selected':'' }}>Inativo</option>
                    </select>
                    <!--end::Select2-->
                    <!--begin::Description-->
                ...
                <div class="card-body pt-0">
                    ...
                    <select class="form-select mb-2" name="galleries[]" data-control="select2" data-placeholder="Selecione uma opção" multiple="multiple">
                        ...
                        @foreach ($galleries as $gallery)
                            <option value="{{$gallery->id}}" {{ $alert->galleries->pluck('id')->contains($gallery->id) ? 'selected':'' }}>{{$gallery->title}}</option>
                        @endforeach
                    </select>
                    <!--end::Select2-->
                    <!--begin::Description-->
            ...
            <div class="card card-flush py-4">
                ...
                <div class="card-body pt-0">
                    ...
                    <div class="mb-10 fv-row">
                        ...
                        <!--end::Label-->
                        <!--begin::Input-->
                        <input type="text" name="title" class="form-control" value="{{ $alert->title }}"/>
                        <!--end::Input-->
                    </div>
                    <!--end::Input group-->
                    <!--begin::Input group-->
                    <div class="mb-10 fv-row">
                        ...
                        <!--end::Label-->
                        <!--begin::Input-->
                        <input type="text" name="slug" class="form-control mb-2" placeholder="Slug" value="{{ $alert->slug }}"/>
                        <!--end::Input-->
                        <!--begin::Description-->
                        <div class="text-muted fs-7">
                            @if($alert->id > 0)
                                <a href="#" target="_blank">
                                    {{url('/')}}/<span id="preview-type">comunicados/</span><span id="preview-slug">{{ $alert->slug }}</span>
                                </a>
                            @else
                                {{url('/')}}/<span id="preview-type">comunicados/</span><span id="preview-slug">{{ $alert->slug }}</span>
                            @endif
                    ...
                    <div class="mb-10 fv-row">
                        ...
                        <!--end::Label-->
                        <textarea id="description" class="form-control mb-2" rows="5" style="resize: none">{!! $alert->description !!}</textarea>
                    </div>
                    <!--end::Input group-->
                </div>
                <!--end::Card header-->
...
@push('scripts')
    ...
    <script>
        @if($alert->photo != null)
            let user_profile = '{{ str_replace('public/', 'storage/', asset($alert->photo)) }}';
    ...
    <script>
        $('.saveEvent').on("submit", function(e){
            ...
            $.ajax({
                url: '{{$alert->id > 0 ? route('admin.alerts.update', $alert->hash_id):route('admin.alerts.store')}}',
                type: 'POST',
                data: form,
                processData: false,
                contentType: false,
                success: function(data){
                    if(data.success){
                        Swal.fire({
                            ...
                            showCancelButton: !0,
                            buttonsStyling: !1,
                            confirmButtonText: "Ver comunicados",
                            cancelButtonText: "Novo comunicado",
                            customClass: {
                            ...
                            if (t.value){
                                ...
                                window.location.href = '{{route('admin.alerts.create')}}'
                            }
                        }))
                    }else{
    ...
    <script>
        ...
        function generateSlug(title){
            ...
            $.ajax({
                url: '{{route('admin.alerts.get.slug')}}',
                type: 'POST',
                data: {title: title},
                dataType: 'json',
                success: function(data){
