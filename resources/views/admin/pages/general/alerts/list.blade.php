...
@section('content')
        ...
        <div class="card-header align-items-center py-5 gap-2 gap-md-5">
            ...
            <div class="card-title">
                ...
                <div class="d-flex align-items-center position-relative my-1">
                    ...
                    <!--end::Svg Icon-->
                    <input type="text" data-kt-ecommerce-product-filter="search" class="form-control form-control-solid w-250px ps-14" placeholder="Procurar comunicado"/>
                </div>
                <!--end::Search-->
            </div>
            <!--end::Card title-->
            ...
            <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                ...
                <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-200px py-4"
                    ...
                    <div class="menu-item px-3">
                        <a href="{{ route('admin.alerts.create') }}" class="menu-link px-3">Adicionar comunicado</a>
                    </div>
                    <!--end::Menu item-->
                    <!--begin::Menu item-->
                    <div class="menu-item px-3">
                        <a href="#" data-bs-toggle="modal" data-bs-target="#modalOrderAlerts" class="menu-link px-3">Ordem de exibição</a>
                    </div>
                    <!--end::Menu item-->
                </div>
                <!--end::Menu-->
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="d-flex align-items-center">
                                <!--begin::Thumbnail-->
                                <a href="{{ route('admin.alerts.edit', $alert->hash_id) }}" class="symbol symbol-50px">
                                    <span class="symbol-label" style="background-image:url('{{$alert->photo == null ? asset('assets/media/svg/files/blank-image.svg') : asset(str_replace('public/','storage/', $alert->photo)) }}');"></span>
                                </a>
                                <!--end::Thumbnail-->
                                <div class="ms-5">
                                    <!--begin::Title-->
                                    <a href="{{ route('admin.alerts.edit', $alert->hash_id) }}" class="text-gray-800 text-hover-primary fs-5 fw-bold" data-kt-ecommerce-product-filter="name">
                                        {{ $alert->title }}
                                    </a>
                                    <a class="d-none" data-kt-ecommerce-product-filter="id">{{ $alert->hash_id }}</a>
                                    <!--end::Title-->
                                </div>
                            </div>
                        </td>
                        <td>{{$alert->slug}}</td>
                        <td>{{ Str::limit(trim(strip_tags(html_entity_decode($alert->description))), 50, '...') }}</td>
                        @php
                            $status = '';
                            $badge = '';
                            /** @var TYPE_NAME $alert */
                        ...
                        <td class="text-center">
                            ...
                            <div
                                ...
                                <!--begin::Menu item-->
                                <div class="menu-item px-3">
                                    <a href="{{route('admin.alerts.edit', $alert->hash_id)}}" class="menu-link px-3">Editar</a>
                                </div>
                                <!--end::Menu item-->
                                <!--begin::Menu item-->
                                <div class="menu-item px-3">
    ...
    <div class="modal fade" id="modalOrderAlerts" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                            ...
                            <div class="row">
                                ...
                                <div class="col-6 fv-row">
                                    ...
                                    <select class="form-select mb-2" id="order-type" data-control="select2" data-hide-search="true" data-placeholder="Selecione uma opção">
                                        <option></option>
                                        <option value="alerts">Comunicados</option>
                                    </select>
                                </div>
                                <!--end::Col-->
                                <!--begin::Col-->
...
@push('scripts')
            ...
            var t, e, n = () => {
                t.querySelectorAll('[data-kt-ecommerce-product-filter="delete_row"]').forEach((t => {
                    t.addEventListener("click", (function (t) {
                        ...
                        Swal.fire({
                            text: "Tem certeza que deseja excluir o comunicado?",
                            icon: "question",
                            showCancelButton: !0,
                            buttonsStyling: !1,
                            ...
                            if(t.value){
                                $.ajax({
                                    method: "DELETE",
                                    url: `{{route('admin.alerts.destroy', '_id_')}}`.replace('_id_', n.querySelector('[data-kt-ecommerce-product-filter="id"]').innerText),
                                    success: function (rtn) {
                                        if (rtn.success === true) {
                                            Swal.fire({
                                                text: "Você excluiu o comunicado",
                                                icon: "success",
                                                buttonsStyling: !1,
    ...
    <script>
        ...
        $('#modalOrderAlerts').on('hide.bs.modal', function (e) {
        ...
        $('#order-type').on('change', function (){
            ...
            $.ajax({
                type: 'GET',
                url: '{{route('admin.alerts.order', '_TYPE_')}}'.replace('_TYPE_', this.value),
            }).then(function (data) {
                if (data.success) {
                    ...
                    $('#order-type-value').select2({
                        dropdownParent: $('#modalOrderAlerts .modal-body') // Ajuste o seletor conforme necessário
                    });
                }else{
        ...
        $('#order-type-value').on('change', function (){
            $.ajax({
                type: 'GET',
                url: '{{route('admin.alerts.order.type', ['type' => '_TYPE_', 'id' => '_ID_'])}}'.replace('_TYPE_', $('#order-type').val()).replace('_ID_', this.value),
            }).then(function (data) {
                if (data.success) {
                    var html;
        ...
        $('#form-order').on('submit', function (e){
            ...
            const id = type === 'alerts' ? 0 : $('#order-type-value').val();
            var form = new FormData(this);
            $.ajax({
                url: '{{route('admin.alerts.order.type', ['type' => '_TYPE_', 'id' => '_ID_'])}}'.replace('_TYPE_', type).replace('_ID_', id),
                type: 'POST',
                data: form,
                processData: false,
                contentType: false,
