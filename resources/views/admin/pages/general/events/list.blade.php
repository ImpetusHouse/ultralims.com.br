...
@section('content')
        ...
        <div class="card-header align-items-center py-5 gap-2 gap-md-5">
            ...
            <div class="card-title">
                ...
                <div class="d-flex align-items-center position-relative my-1">
                    ...
                    <!--end::Svg Icon-->
                    <input type="text" data-kt-ecommerce-product-filter="search" class="form-control form-control-solid w-250px ps-14" placeholder="Procurar evento"/>
                </div>
                <!--end::Search-->
            </div>
            <!--end::Card title-->
            ...
            <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                ...
                <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-200px py-4"
                    ...
                    <div class="menu-item px-3">
                        <a href="{{ route('admin.events.create') }}" class="menu-link px-3">Adicionar evento</a>
                    </div>
                    <!--end::Menu item-->
                    <!--begin::Menu item-->
                    <div class="menu-item px-3">
                        <a href="#" data-bs-toggle="modal" data-bs-target="#modalOrderEvents" class="menu-link px-3">Ordem de exibição</a>
                    </div>
                    <!--end::Menu item-->
                </div>
                <!--end::Menu-->
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="d-flex align-items-center">
                                <!--begin::Thumbnail-->
                                <a href="{{ route('admin.events.edit', $event->hash_id) }}" class="symbol symbol-50px">
                                    <span class="symbol-label" style="background-image:url('{{$event->photo_square == null ? asset('assets/media/svg/files/blank-image.svg') : asset(str_replace('public/','storage/', $event->photo_square)) }}');"></span>
                                </a>
                                <!--end::Thumbnail-->
                                <div class="ms-5">
                                    <!--begin::Title-->
                                    <a href="{{ route('admin.events.edit', $event->hash_id) }}" class="text-gray-800 text-hover-primary fs-5 fw-bold" data-kt-ecommerce-product-filter="name">
                                        {{ $event->title }}
                                    </a>
                                    <a class="d-none" data-kt-ecommerce-product-filter="id">{{ $event->hash_id }}</a>
                                    <!--end::Title-->
                                </div>
                            </div>
                        </td>
                        <td>{{$event->slug}}</td>
                        <td>{{ Str::limit(trim(strip_tags(html_entity_decode($event->content))), 50, '...') }}</td>
                        @php
                            $status = '';
                            $badge = '';
                            /** @var TYPE_NAME $event */
                        ...
                        <td class="text-center">
                            ...
                            <div
                                ...
                                <!--begin::Menu item-->
                                <div class="menu-item px-3">
                                    <a href="{{route('admin.events.edit', $event->hash_id)}}" class="menu-link px-3">Editar</a>
                                </div>
                                <!--end::Menu item-->
                                <!--begin::Menu item-->
                                <div class="menu-item px-3">
    ...
    <div class="modal fade" id="modalOrderEvents" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
...
@push('scripts')
                    ...
                    t.addEventListener("click", (function (t) {
                        ...
                        Swal.fire({
                            ...
                            if(t.value){
                                $.ajax({
                                    method: "DELETE",
                                    url: `{{route('admin.events.destroy', '_id_')}}`.replace('_id_', n.querySelector('[data-kt-ecommerce-product-filter="id"]').innerText),
                                    success: function (rtn) {
                                        if (rtn.success === true) {
                                            Swal.fire({
                                                text: "Você excluiu o evento",
                                                icon: "success",
                                                buttonsStyling: !1,
    ...
    <script>
        ...
        $('#modalOrderEvents').on('hide.bs.modal', function (e) {
        ...
        $('#order-type').on('change', function (){
            ...
            $.ajax({
                type: 'GET',
                url: '{{route('admin.events.order', '_TYPE_')}}'.replace('_TYPE_', this.value),
            }).then(function (data) {
                if (data.success) {
                    ...
                    $('#order-type-value').select2({
                        dropdownParent: $('#modalOrderEvents .modal-body') // Ajuste o seletor conforme necessário
                    });
                }else{
        ...
        $('#order-type-value').on('change', function (){
            $.ajax({
                type: 'GET',
                url: '{{route('admin.events.order.type', ['type' => '_TYPE_', 'id' => '_ID_'])}}'.replace('_TYPE_', $('#order-type').val()).replace('_ID_', this.value),
            }).then(function (data) {
                if (data.success) {
                    var html;
        ...
        $('#form-order').on('submit', function (e){
            ...
            const id = type === 'events' ? 0 : $('#order-type-value').val();
            var form = new FormData(this);
            $.ajax({
                url: '{{route('admin.events.order.type', ['type' => '_TYPE_', 'id' => '_ID_'])}}'.replace('_TYPE_', type).replace('_ID_', id),
                type: 'POST',
                data: form,
                processData: false,
                contentType: false,
