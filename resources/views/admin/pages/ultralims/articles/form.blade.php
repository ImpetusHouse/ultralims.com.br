                                        ...
                                        <div class="text-muted fs-7">
                                            @if($article->id > 0)
                                                <a href="#" target="_blank">
                                                    {{url('/')}}/<span id="preview-type">cliente-ultra/blog/</span><span id="preview-slug">{{ $article->slug }}</span>
                                                </a>
                                            @else
                                                {{url('/')}}/<span id="preview-type">cliente-ultra/blog/</span><span id="preview-slug">{{ $article->slug }}</span>
                                            @endif
                                        ...
                                        @endphp
                                            ...
                                            <div class="timeline-item">
                                                ...
                                                <div class="timeline-content d-flex">
                                                    ...
                                                    <a href="{{ route('admin.ultralims.articles.undo', [$article->hash_id, $oldArticle->id]) }}">
...
@push('scripts')
    ...
    <script>
        $('.saveArticle').on("submit", function(e){
            ...
            $.ajax({
                url: '{{ $article->id > 0 ? route('admin.ultralims.articles.update', $article->hash_id):route('admin.ultralims.articles.store') }}',
                type: 'POST',
                data: form,
                processData: false,
                contentType: false,
                    ...
                    if(data.success){
                        Swal.fire({
                            ...
                            allowOutsideClick: false
                             ...
                             if (t.value){
                                 window.location.href = '{{route('admin.ultralims.articles.index')}}'
                             }else{
                                 window.location.href = '{{route('admin.ultralims.articles.create')}}'
                             }
                        }))
                    }else{
    ...
    <script>
        ...
        function generateSlug(title){
            ...
            $.ajax({
                url: '{{route('admin.ultralims.articles.get.slug')}}',
                type: 'POST',
                data: {title: title},
                dataType: 'json',
                success: function(data){
    ...
    <script>
        ...
        $('#saveCategory').on("click", function(e){
            ...
            if(val.trim() != ''){
                if(ModalCategoryTypeSend == 'store'){
                    $.ajax({
                        method: "POST",
                        url: "{{route('admin.ultralims.articles.categories.store')}}",
                        data: {name: val},
                    ...
                    $.ajax({
                        type: "PUT",
                        url: "{{route('admin.ultralims.articles.categories.update', '_id_')}}".replace('_id_', ModalCategoryChange),
        ...
        function deleteCategory(category_id){
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        method: "DELETE",
                        url: "{{route('admin.ultralims.articles.categories.destroy', '_id_')}}".replace('_id_', category_id),
                        data: {id: category_id},
                        success: function(rtn){
                            if(rtn.success){
                                if(rtn.reloaded){
        ...
        function fetchSelect2(){
            ...
            $.ajax({
                type: 'GET',
                data: {ajaxJson: true},
                url: '{{route('admin.ultralims.articles.categories.index')}}',
            }).then(function (data) {
