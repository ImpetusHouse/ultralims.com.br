...
class ArticlesController extends Controller
{
    public function index(){
        ...
        $title = "Publicações";
        $breadcrumbs = [
            ['title' => $title, 'url' => route('admin.articles.index')]
        ];
        $data = [
            'title' => $title,
    ...
    public function create(){
        ...
        $breadcrumbs = [
            ['title' => 'Publicações', 'url' => route('admin.articles.index')],
            ['title' => $title, 'url' => route('admin.articles.create')],
        ];
        $data = [
            'title' => $title,
            'breadcrumbs' => $breadcrumbs,
        ...
        return view('admin.pages.articles.form', $data);
    }
    ...
    public function edit(Article $article){
        ...
        $breadcrumbs = [
            ['title' => 'Publicações', 'url' => route('admin.articles.index')],
            ['title' => $title, 'url' => route('admin.articles.edit', $article->hash_id)],
        ];
        $data = [
            'title' => $title,
            'breadcrumbs' => $breadcrumbs,
    ...
    public function save($article, $request, $action){
        ...
        try{
            ...
            if ($action == 'update'){
                ...
                $article->galleries()->sync($oldArticle->galleries->pluck('id'));
                $oldArticle->delete();
            }else{
                $article->version = 1;
            ...
            if(!empty($request->galleries) or $request->galleries != null){
                ...
                //Save categories pivot
                $article->galleries()->sync($galleries);
            }
    ...
    public function undo(Article $article, $id){
        ...
        $newArticle->galleries()->sync($undoArticle->galleries->pluck('id'));
        $article->delete();

        return redirect()->route('admin.articles.edit', $newArticle->hash_id);
    }
